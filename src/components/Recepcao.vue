    ...
    <v-container>
      ...
      <v-form @submit.prevent="cadastrarCliente">
        ...
        <v-btn
          ...
          v-bind:disabled="cliente.idade >= 30 || cliente.nome.length === 0 || cliente.raca.length === 0 ||cliente.idade.length === 0 || cliente.servico.tipo.length === 0 "
          >
            Cadastrar cliente
          </v-btn>
...
export default {
  ...
  methods: {
    ...
    async buscarServicos() {
      ...
      // data.map(servico => {
      //   this.servicos.push(`${servico.nome} (${servico.preco})`);
      // });
      return data;
    },
    cadastrarCliente() {
